...
/// (<c>&lt;file&gt;.viskatweak.bak</c>) so <see cref="Restore"/> can always put the original back.
/// </summary>
public static class OptimizerService
{
    private const string BackupSuffix = ".viskatweak.bak";

    /// <summary>Backups written before the rename. Still honoured so nobody loses their original.</summary>
    private static readonly string[] LegacySuffixes = { ".fivemtweaks.bak", ".rigbooster.bak" };

    /// <summary>The newest suffix present for this file, or empty if there is no backup at all.</summary>
    ...
    {
        // Nothing held back. Every quality dial at its floor, every distance at zero, and the
        // things the other profiles leave alone - water, distant shadows, decals, mirrors - turned
        // off outright. It looks bad on purpose; it is for a machine that cannot hold 25 otherwise.
        PresetProfile.Potato => new()
        {
            ...
            ["MSAA"] = "0", ["SamplingMode"] = "0", ["FXAA_Enabled"] = "false",
            ...
            ["Shadow_Distance"] = "0.000000", ["Shadow_SplitZStart"] = "0.930000",
            ["Shadow_SplitZEnd"] = "0.890000", ["Shadow_aircraftExpWeight"] = "0.990000",
            ["Reflection_MipBlur"] = "false", ["Lighting_FogVolumes"] = "false",
            ["Shader_SSA"] = "false", ["DeferredLighting"] = "false",

            ["HdStreamingInFlight"] = "false",
            ["MaxLodScale"] = "0.000000", ["LodScale"] = "0.000000",
            ["PedLodBias"] = "0.000000", ["VehicleLodBias"] = "0.000000",
            ["CityDensity"] = "0.000000",
            ["PedVarietyMultiplier"] = "0.000000", ["VehicleVarietyMultiplier"] = "0.000000",
    ...
    public static ApplyResult ApplyProfile(GameTarget game, PresetProfile profile)
    {
        ...
        try
        {
            ...
            JournalService.Record("Settings", $"{profile} preset applied", game.SettingsPath,
                $"{changed} values changed in {game.DisplayName}. Original kept as .viskatweak.bak.");
    ...
    {
        var bak = FindBackup(game.SettingsPath);
        if (bak.Length == 0) return new(false, "No backup exists for this game yet.", 0);
        try
        {
    ...
    {
        // Never overwrite an existing backup, under any of the names we have used - the first one
        // taken is the pristine file and the only one worth keeping.
